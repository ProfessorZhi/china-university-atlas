...
V5.1.0 是 **Windows V5 的仓库化迁移版**，不是“全国所有校区均已补齐”的版本。重建后的完整数据与 Windows V5 的规范化 JSON SHA-256 一致，363 份地图边界的压缩前 SHA-256 也一致。核对记录在 [migration-verification.json](reports/migration-verification.json)。HTML 因可复现打包和版本标记调整而重新生成，不冒称与旧 HTML 字节相同。

| 内容 | 当前快照 |
|---|---:|
| 普通高校主体 / 成人高校主体 | 2,952 / 244 |
| 校区、办学地点记录 | 6,762 |
| 有至少一个地点关联的普通高校 | 2,882 |
| 仍无具体地点 / 只有市级地点的普通高校 | 70 / 150 |
| 有地点记录的县区 / 源数据县区轮廓 | 1,201 / 2,840 |
| 官网校区依据 / 第三方档案 / 历史位置记录 | 42 / 4,864 / 1,856 |

数据日期为 2026-09-08；学校主体来自教育部 2026 名录的 CSV 镜像。**地点记录数不等于互不重复的独立校园数。** 官网校区关联也不表示精确坐标、现用状态、招生专业和招生层次全部核验。港澳台学校名录、台湾市县轮廓尚未系统补全；不设普通县区的城市保留源数据可用的本级轮廓。
...
- `data/universities.jsonl`、`data/campuses.jsonl`：每行一个实体；`uid` 关联学校 `id`。`regions.json`、`metadata.json` 与边界压缩文件共同组成地图数据。
...
常规构建仅用 Python 标准库、无网络；同一组输入产生同一 HTML。CI 检查数据、隐私扫描与重建一致性。缺坐标不填县城中心点；缺地点不标成“当地无高校”；新增校区必须保留来源与核验状态。详见 [AGENTS.md](AGENTS.md)、[数据使用说明](NOTICE.md) 和 [迁移说明](docs/migration.md)。
