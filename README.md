...
| 市 | 346 | 71 | 275 | 0 |
| 区县 | 1,220 | 280 | 940 | 0 |
| **合计** | **1,598** | **382** | **1,216** | **0** |

`resolved_strong` 仍只依赖可直接比较的官方证据；`resolved_policy` 包含**显式项目政策**（唯一候选 / 本科>专科 / 校区优先 / 公办优先 / 唯一被覆盖 / 跨榜体系优先级 / 最终稳定兜底）。V5.9.1 按产品要求取消有候选单元的留白：不同榜单体系仍不把裸名次伪装成同一把尺，而是明确标记 policy 后给出唯一展示 winner。当前 **unresolved = 0、legacy 赢家 = 0、未经审计赢家 = 0**；最终稳定兜底不是外部排名证据。

前端与 winner hotfix 详见 [V5.9.1 说明](docs/v5.9.1.md) 与 [V5.9 说明](docs/v5.9.md)；此前视觉变化详见 [V5.8 验收文档](docs/v5.8.md)（含 Before/After、测试矩阵、视觉决策与**已知未闭合项**）与 [V5.7 说明](docs/v5.7.md)；Best University 语义见 [V5.6 说明](docs/v5.6.md) 与 [独立复核](docs/v5.6-redteam.md)；地点证据分层见 [V5.2 说明](docs/v5.2.md)。
