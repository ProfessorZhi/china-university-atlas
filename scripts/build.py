...
def build():
    ...
    manifest={'complete':True,'maps':len(maps),'version':version,'preparedAt':'2026-09-08','source':'See data/sources.json','schoolDataComplete':'现有名录主体已嵌入；校区及办学状态仍有缺口，见覆盖统计。'}
    values={'__STYLES__':(ROOT/'src/styles.css').read_text(encoding='utf-8'),'__APP__':(ROOT/'src/app.js').read_text(encoding='utf-8'),'__SCHOOL_DATA__':json_text(data),'__GEO_DATA__':base64.b64encode(geometry).decode(),'__MANIFEST__':json_text(manifest)}
    ...
    if args.zip:
        ...
        with zipfile.ZipFile(target,'w',compression=zipfile.ZIP_DEFLATED,compresslevel=9) as archive:
            for path in sorted(set(selected)):
                ...
                item=zipfile.ZipInfo(name,(2026,9,8,0,0,0));item.compress_type=zipfile.ZIP_DEFLATED;item.external_attr=0o644<<16
