...
def refresh_derived(data, version, date='2026-09-09'):
    ...
    local_boundary_campus_ids={r['uid'] for r in campuses if r.get('d') or (r.get('p'),r.get('c')) in terminal_keys}
    assoc_ids={a['uid'] for a in associations};evidence_ids=campus_ids|assoc_ids
    local_boundary_evidence_ids=local_boundary_campus_ids|assoc_ids
    ...
    city_only=[{'uid':u['id'],'学校':u['u'],'省份':u['p'],'城市':u['c'],'层次':u['level'],'缺口':'已有办学地点，但当前记录尚未映射到最低可用法定行政边界'} for u in active if u['id'] in campus_ids and u['id'] not in local_boundary_campus_ids]
