...
def refresh_derived(data, version, date='2026-09-09'):
    ...
    campus_gap_records=[r for r in campuses if not campus_has_local_boundary(r)]
    campus_location_gaps=[{'campus_id':r.get('id'),'uid':r.get('uid'),'学校':r.get('u'),'省份':r.get('p'),'城市':r.get('c'),'校区':r.get('campus'),'地址':r.get('address'),'来源类型':r.get('sourceKind'),'已核验':bool(r.get('verified')),'状态':r.get('status')} for r in campus_gap_records]
    gap_source_counts={kind:sum(r.get('sourceKind')==kind for r in campus_gap_records) for kind in ['official','government','corroborated','profile','historical']}
    ...
    st.update(
        ...
        campusRecordsWithLowestLegalBoundary=len(campuses)-len(campus_gap_records),campusRecordsOnlyCityLocation=len(campus_gap_records),campusRecordsPendingBoundary=sum((r.get('p'),r.get('c')) in pending_boundary_keys for r in campuses),
        campusLocationGapsVerified=sum(bool(r.get('verified')) for r in campus_gap_records),campusLocationGapsOfficial=gap_source_counts['official'],campusLocationGapsGovernment=gap_source_counts['government'],campusLocationGapsCorroborated=gap_source_counts['corroborated'],campusLocationGapsProfile=gap_source_counts['profile'],campusLocationGapsHistorical=gap_source_counts['historical'],
    ...
    data['campusLocationGapBreakdown']={'total':len(campus_gap_records),'verified':st['campusLocationGapsVerified'],'bySourceKind':gap_source_counts}
    ...
    for p in data.get('provinceCities',{}):
        ...
        province_gap_records=[r for r in rs if not campus_has_local_boundary(r)]
        coverage.append({'province':p,'schools':len(us),'registrySchools':sum(u['p']==p for u in ordinary),'inactiveSchools':sum(u['p']==p for u in inactive),'locatedSchools':sum(u['id'] in evidence_ids for u in us),'campusLocatedSchools':sum(u['id'] in campus_ids for u in us),'districtEvidenceOnly':sum(u['id'] in assoc_ids and u['id'] not in campus_ids for u in us),'missingSchools':sum(u['id'] not in evidence_ids for u in us),'pendingBoundarySchools':sum(u['id'] in pending_boundary_ids for u in us),'campuses':len(rs),'campusRecordsWithLowestLegalBoundary':len(rs)-len(province_gap_records),'campusLocationGaps':len(province_gap_records),'campusLocationGapsVerified':sum(bool(r.get('verified')) for r in province_gap_records),'campusLocationGapsProfile':sum(r.get('sourceKind')=='profile' for r in province_gap_records),'districtAssociations':len(aa),'districts':len(local_units),'allDistricts':len(all_local_units),'verifiedRecords':sum(bool(r.get('verified')) for r in rs)})
