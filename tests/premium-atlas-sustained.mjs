...
async function main(){
 ...
 for(let i=0;i<36;i++){
   await evaljs(`(()=>{const m=document.getElementById('map'),r=m.getBoundingClientRect();for(let j=0;j<2;j++)m.dispatchEvent(new WheelEvent('wheel',{deltaY:-3.5,clientX:r.left+r.width*.54,clientY:r.top+r.height*.51,bubbles:true,cancelable:true}));return true;})()`);
   await wait(12);
 }
 ...
 const checks={sustainedAcrossFrames:frames>=8,eventsDelivered:events===72,noLayoutDuringGesture:layoutDuring===0,noPrematureIdleLayout:layoutBeforeIdle===0,oneIdleLayout:layoutAfter===1,labelsNotMovedIndividually:during.labelTransforms===0,labelsSuspended:during.labelOpacity==='0',blurSuspended:/none/.test(during.headerBlur),zeroLeaders:during.leaders===0&&after.leaders===0,transformWorkLight:maxWork<5,offline:http.length===0,runtimeClean:errors.length===0};
 const result={pass:Object.values(checks).every(Boolean),checks,gestureMs,events,frames,layoutDuring,layoutBeforeIdle,layoutAfter,maxTransformWorkMs:Number(maxWork.toFixed(3)),before,during,after,httpRequests:http,runtimeErrors:errors,artifact:'dist/china-university-atlas.html'};
